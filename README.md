...
<a href="https://apps.apple.com/us/app/apex-iptv-player/id6779551584">
  <img src="https://toolbox.marketingtools.apple.com/api/v2/badges/download-on-the-app-store/black/en-us?releaseDate=1700000000" alt="Download Apex on the App Store" height="48">
</a>

<br><br>

[![App Store](https://img.shields.io/badge/Download-App%20Store-0A84FF?logo=apple&logoColor=white&labelColor=1f1f2e)](https://apps.apple.com/us/app/apex-iptv-player/id6779551584)
...
- **Deep links** — open a title straight from a URL: `apex://movie/{tmdbId}` and `apex://series/{tmdbId}`
...
Apex is a single SwiftUI codebase that adapts to each platform's idioms — including a
...
Apex ships with three interchangeable engines, ordered into a **priority list** in
...
Prefer a third-party app? Apex can hand streams off to an **external player** —
**Infuse** or **VLC** — via their deep-link APIs, selectable in **Settings**.
Downloads always play in Apex, and playback falls back to the built-in player when
...
Apex follows a clean, layered SwiftUI architecture:
...
```
Apex/
├── ApexApp.swift            App entry point & SwiftData container
...
ApexTests/                   Unit & integration tests (Swift Testing)
ApexUITests/                 UI automation tests (XCTest)
...
The easiest way to use Apex is to [**download it from the App Store**](https://apps.apple.com/us/app/apex-iptv-player/id6779551584) — available on iPhone, iPad, Mac, Apple TV, and Apple Vision Pro. To build from source, follow the steps below.
...
Select the **Apex** scheme and a target destination (iPhone, Mac, Apple TV, or Vision
Pro), then build and run (`⌘R`). On first launch, sign in with your Xtream credentials
or import an M3U playlist, and Apex will sync your catalog.
...
(`DEVELOPMENT_TEAM`) and bundle identifier (`com.TBD.apex`). Simulator builds run as
...
Every key is optional — Apex builds and runs fine with an empty `.env` or none at all.
...
Apex has an extensive test suite split across unit/integration tests (**Swift Testing**)
and UI automation (**XCTest**).

| Target | Framework | Coverage |
|---|---|---|
| `ApexTests` | Swift Testing | DTO decoding, URL building, API client & retry, models, sort options, sync progress & content sync, playable media, player settings, Trakt token store, content organizing, **M3U parser/classifier/sync**, **OMDb client**, **Next Episode resolver**, **Gzip file streaming** |
| `ApexUITests` | XCTest | App launch & performance, login flow, tab navigation, playlist detail, settings, **M3U playlist import flow** |
...
helpers in `ApexTests/Helpers/TestHelpers.swift` provide an in-memory `ModelContainer`
...
Apex is a **player only** — it ships with no channels, streams, playlists, or media of
...
We do **not** condone or support piracy. Use Apex only with content you are legally
...
Apex is free software, licensed under the **GNU Affero General Public License v3.0
(AGPL-3.0)**. See [`LICENSE`](LICENSE) for the full text.

In short — you are free to use, study, modify, and redistribute Apex, but **any
...
modified version of Apex as a network service, you must make your modified source
