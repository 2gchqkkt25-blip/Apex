...
struct ModelTests {
    ...
    @Test func `category ID construction`() {
        ...
        let category = Apex.Category(apiId: "42", name: "Action", parentId: 0, type: .vod, playlist: playlist)
    ...
    @Test func `category type round trip`() {
        let playlist = Playlist(name: "P", serverURL: "http://x.com", username: "u", password: "p")
        let live = Apex.Category(apiId: "1", name: "Live", parentId: 0, type: .live, playlist: playlist)
    ...
    @Test func `category upsert via unique attribute`() throws {
        ...
        let cat = Apex.Category(apiId: "1", name: "Original", parentId: 0, type: .vod, playlist: playlist)
        ctx1.insert(cat)
        try ctx1.save()

        let ctx2 = ModelContext(container)
        ctx2.autosaveEnabled = false
        let cat2 = Apex.Category(apiId: "1", name: "Updated", parentId: 0, type: .vod, playlist: playlist)
        cat2.id = cat.id // Same unique ID
        ctx2.insert(cat2)
        try ctx2.save()

        let expectedApiId = "1"
        let fetch = FetchDescriptor<Apex.Category>(predicate: #Predicate { $0.apiId == expectedApiId })
