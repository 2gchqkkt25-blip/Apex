...
struct SortOptionTests {
    ...
    @Test func `storage key constants`() {
        #expect(SortStorageKey.liveCategories == "apex.sort.live.categories")
        #expect(SortStorageKey.liveContent == "apex.sort.live.content")
        #expect(SortStorageKey.movieCategories == "apex.sort.movies.categories")
        #expect(SortStorageKey.movieContent == "apex.sort.movies.content")
        #expect(SortStorageKey.seriesCategories == "apex.sort.series.categories")
        #expect(SortStorageKey.seriesContent == "apex.sort.series.content")
    }

    // MARK: - Helpers

    private func makeUnsortedCategories() -> [Apex.Category] {
        let playlist = Playlist(name: "P", serverURL: "http://x.com", username: "u", password: "p")
        return [
            Apex.Category(apiId: "2", name: "B Category", parentId: 0, type: .vod, playlist: playlist),
            Apex.Category(apiId: "1", name: "A Category", parentId: 0, type: .vod, playlist: playlist),
            Apex.Category(apiId: "4", name: "D Category", parentId: 0, type: .vod, playlist: playlist),
            Apex.Category(apiId: "3", name: "C Category", parentId: 0, type: .vod, playlist: playlist)
        ].enumerated().map { idx, cat in cat.sortOrder = idx * 2; return cat }
    }

    private func makeDuplicateNameCategories() -> [Apex.Category] {
        let playlist = Playlist(name: "P", serverURL: "http://x.com", username: "u", password: "p")
        return [
            Apex.Category(apiId: "1", name: "Same Name", parentId: 0, type: .vod, playlist: playlist),
            Apex.Category(apiId: "2", name: "Same Name", parentId: 0, type: .vod, playlist: playlist),
            Apex.Category(apiId: "3", name: "Z Alone", parentId: 0, type: .vod, playlist: playlist)
