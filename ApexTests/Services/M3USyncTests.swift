...
struct M3USyncTests {
    ...
    @Test func `syncs mixed playlist into unified models`() async throws {
        ...
        let categories = try context.fetch(FetchDescriptor<Apex.Category>())
    ...
    @Test func `re-sync is idempotent and preserves user state`() async throws {
        ...
        do {
            ...
            let category = try #require(try context.fetch(FetchDescriptor<Apex.Category>()).first { $0.name == "News" })
        ...
        #expect(try context.fetchCount(FetchDescriptor<Apex.Category>()) == 6)
    ...
    @Test func `re-sync prunes content the playlist no longer contains`() async throws {
        ...
        #expect(try context.fetchCount(FetchDescriptor<Apex.Category>()) == 3) // News, General, VOD | Action
        ...
        let categoryNames = try Set(context.fetch(FetchDescriptor<Apex.Category>()).map(\.name))
    ...
    @Test func `syncs a 100k-entry playlist in bounded time`() async throws {
        ...
        #expect(try context.fetchCount(FetchDescriptor<Apex.Category>()) == 70)
