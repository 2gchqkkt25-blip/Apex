...
Thanks for your interest in improving Apex! This guide covers everything you need to
...
> **A note on content** — Apex is a *player only*. It ships with no channels, streams,
...
Be respectful, constructive, and welcoming. We want Apex to be a friendly place for
...
Apex is a **single, platform-adaptive SwiftUI codebase** targeting iOS, iPadOS, macOS,
...
Apex ships in **English and German** via String Catalogs (`.xcstrings`). If your change
...
Apex has an extensive suite split across **Swift Testing** (`ApexTests`) and **XCTest**
UI automation (`ApexUITests`). **Run the suite before opening a PR**, and add or update
...
`ApexTests/Helpers/TestHelpers.swift` provide an in-memory `ModelContainer` and JSON
...
Apex follows the [Conventional Commits](https://www.conventionalcommits.org/) format:
...
By contributing to Apex, you agree that your contributions will be licensed under the
