...
> Last updated: **July 7, 2026 (late evening)**  
> Status: **Generic providers** — offset-honest `xmltv.php` bulk sync works. **StreamInfinity test panel** — 14 external epgshare01 feeds + structural West/Pacific `+3h` insert + **single-pass parse** + **playlist-bundled fast sync**. See § "StreamInfinity panel — status" below.
...
1. **Never clear the entire `EPGListing` store at sync start** — blanks the UI; if sync fails, guide stays empty. Upsert; prune expired only after success. **Exception (Jul 7):** external epgshare01 full replace at the start of `syncExternalEPG` is intentional — stale shifted rows from the old provider feed must not coexist with fresh external data. Browse during that window may be empty until the first feed saves; mid-sync `refreshGeneration` mitigates this.  
2. **Never block browse on `EPGSyncGate` for Xtream API sync** — gate is only for heavy XMLTV parse.  
