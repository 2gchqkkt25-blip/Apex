...
final class LiveStream {
    ...
    /// The channel's logo as a properly percent‑encoded URL, or nil when no icon
    /// is available.  Xtream providers embed credentials in the path; raw
    /// `URL(string:)` silently fails on spaces and special characters.
    var iconURL: URL? {
        guard let raw = streamIcon?.trimmingCharacters(in: .whitespacesAndNewlines), !raw.isEmpty else { return nil }
        // Protocol-relative URLs (`//host/path`) are common from IPTV panels.
        if raw.hasPrefix("//") {
            return URL(string: "https:" + raw)
        }
        // If it's already a valid URL, use it as‑is.
        if let url = URL(string: raw), url.scheme != nil { return url }
        // Otherwise, percent‑encode the path portion (but preserve the scheme +
        // host if present, so `http://server/path with spaces/logo.png` works).
