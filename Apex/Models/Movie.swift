...
final class Movie {
    ...
    /// Percent‑encoded poster / thumbnail URL.  Xtream providers embed
    /// credentials in image paths; raw `URL(string:)` fails on spaces.
    var iconURL: URL? {
        guard let raw = streamIcon, !raw.isEmpty else { return nil }
