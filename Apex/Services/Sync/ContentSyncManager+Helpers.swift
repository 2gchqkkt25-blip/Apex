...
extension ContentSyncManager {
    ...
    func applySeriesFields(from dto: XtreamSeries, to series: Series, playlistPrefix: String, serverURL: String) {
        series.name = dto.name ?? ""
        series.cover = absoluteIconURL(from: dto.cover, serverURL: serverURL)
    ...
    ///
    /// Xtream APIs return `stream_icon` / `streamIcon` as a path relative to the
    /// server root (e.g. `images/logo.png`). If the value is already a full URL
    /// with a scheme it is returned unchanged; otherwise `serverURL` is prepended
    /// so the icon is fetchable by `CachedAsyncImage` / `URLSession`.
    func absoluteIconURL(from raw: String?, serverURL: String) -> String? {
