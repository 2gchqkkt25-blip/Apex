...
extension ContentSyncManager {
    ...
    func fetchTMDBMovieDetails(tmdbId: Int) async throws -> TMDBTitleDetails? {
        ...
        guard client.isConfigured else {
            Logger.network.warning("[TMDBEnrich] TMDBClient not configured — cannot fetch movie details for tmdbId \(tmdbId)")
            return nil
        }
    ...
    func fetchTMDBTVDetails(tmdbId: Int) async throws -> TMDBTitleDetails? {
        let client = TMDBClient.shared
        guard client.isConfigured else {
            Logger.network.warning("[TMDBEnrich] TMDBClient not configured — cannot fetch TV details for tmdbId \(tmdbId)")
            return nil
        }
    ...
    func enrichMovie(id: String, tmdbId: Int) async {
        ...
        do {
            guard let result = try await fetchTMDBMovieDetails(tmdbId: tmdbId) else {
                Logger.network.warning("[TMDBEnrich] fetchTMDBMovieDetails returned nil for movie \(id) (tmdbId \(tmdbId)) — client not configured or invalid response")
                return
            }
            details = result
        } catch {
            Logger.network.error("[TMDBEnrich] Failed to fetch TMDB details for movie \(id) (tmdbId \(tmdbId)): \(error.localizedDescription)")
            return
        }
        ...
        guard let movie = try? context.fetch(descriptor).first else {
            Logger.network.warning("[TMDBEnrich] Movie \(id) not found in store for enrichment")
            return
        }
        applyMovieDetails(details, to: movie, context: context)
        do {
            try context.save()
            Logger.network.info("[TMDBEnrich] Enriched movie \(id) (tmdbId \(tmdbId))")
        } catch {
            Logger.network.error("[TMDBEnrich] Failed to save enrichment for movie \(id): \(error.localizedDescription)")
        }
    }
    ...
    func enrichSeries(id: String, tmdbId: Int) async {
        let details: TMDBTitleDetails
        do {
            guard let result = try await fetchTMDBTVDetails(tmdbId: tmdbId) else {
                Logger.network.warning("[TMDBEnrich] fetchTMDBTVDetails returned nil for series \(id) (tmdbId \(tmdbId)) — client not configured or invalid response")
                return
            }
            details = result
        } catch {
            Logger.network.error("[TMDBEnrich] Failed to fetch TMDB details for series \(id) (tmdbId \(tmdbId)): \(error.localizedDescription)")
            return
        }
        let context = ModelContext(modelContainer)
        context.autosaveEnabled = false
        ...
        guard let series = try? context.fetch(descriptor).first else {
            Logger.network.warning("[TMDBEnrich] Series \(id) not found in store for enrichment")
            return
        }
        applySeriesDetails(details, to: series, context: context)
        do {
            try context.save()
            Logger.network.info("[TMDBEnrich] Enriched series \(id) (tmdbId \(tmdbId))")
        } catch {
            Logger.network.error("[TMDBEnrich] Failed to save enrichment for series \(id): \(error.localizedDescription)")
        }
    }
...
nonisolated func applySeriesDetails(
    ...
    // Extract the first YouTube trailer so the detail screen can offer a
    // one‑tap "Watch Trailer" button, matching the Movie detail screen.
    if series.youtubeTrailer == nil,
       let firstTrailer = details.videos.first(where: { $0.type.lowercased() == "trailer" })
    {
        series.youtubeTrailer = firstTrailer.key
    }
