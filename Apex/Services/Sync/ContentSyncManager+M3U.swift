...
private final nonisolated class M3UImportState {
    /// Base URL of the M3U source, used to resolve relative tvg-logo paths.
...
extension ContentSyncManager {
    ...
    func performM3USync(playlist: Playlist, playlistId: UUID, progress: SyncProgress?) async throws {
        ...
        let summary = try importM3UFile(fileURL, playlistId: playlistId, serverURL: serverURL, progress: progress)
    ...
    private func importM3UFile(_ fileURL: URL, playlistId: UUID, serverURL: String, progress: SyncProgress?) throws -> M3UImportSummary {
        let state = M3UImportState(serverURL: serverURL)
    ...
    private func importLive(_ entries: [M3UEntry], playlistId: UUID, state: M3UImportState, context: ModelContext) {
        ...
        for (entry, id) in zip(entries, ids) {
            ...
            stream.streamIcon = absoluteIconURL(from: entry.logo, serverURL: state.serverURL)
    ...
    private func importMovies(_ entries: [M3UEntry], playlistId: UUID, state: M3UImportState, context: ModelContext) {
        ...
        for (entry, id) in zip(entries, ids) {
            ...
            movie.streamIcon = absoluteIconURL(from: entry.logo, serverURL: state.serverURL)
    ...
    private func importEpisodes(
        ...
        for (index, item) in entries.enumerated() {
            ...
            if series.cover == nil {
                series.cover = absoluteIconURL(from: entry.logo, serverURL: state.serverURL)
            }
            ...
            episode.movieImage = absoluteIconURL(from: entry.logo, serverURL: state.serverURL)
