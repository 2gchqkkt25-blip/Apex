...
struct CategoryTile: View {
    ...
    private var background: some View {
        #if os(tvOS)
            // A solid white layer crossfaded over the resting fill — keeping
...
// MARK: - Poster category tiles (tvOS Search)

/// A poster-style category tile with artwork from a title in that category.
struct CategoryPosterTile: View {
    let name: String
    let imageURL: URL?
...
/// Poster grid of categories for tvOS Search — each tile navigates to the full
/// category grid via `NavigationLink(value:)`.
struct CategoryPosterGridSection: View {
    let title: LocalizedStringKey
    let categories: [Category]
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            LazyVGrid(columns: columns, spacing: PosterCardMetrics.gridSpacing) {
                ForEach(categories) { category in
                    NavigationLink(value: category) {
                        CategoryPosterTile(name: category.name, imageURL: artwork[category.id])
                    }
                    .posterCardButtonStyle()
                }
            }
...
/// Resolves a representative poster URL for each category on a background context.
enum CategoryArtwork {
    static func posterURLs(for categories: [Category], in container: ModelContainer) async -> [String: URL] {
        await Task.detached(priority: .utility) {
            posterURLs(for: categories, in: container)
        }.value
    }

    private static func posterURLs(for categories: [Category], in container: ModelContainer) -> [String: URL] {
        let context = ModelContext(container)
        var map: [String: URL] = [:]
        map.reserveCapacity(categories.count)
        for category in categories {
            if let url = posterURL(for: category, context: context) {
                map[category.id] = url
            }
        }
        return map
    ...
    private static func posterURL(for category: Category, context: ModelContext) -> URL? {
        let categoryId = category.id
        switch category.typeRaw {
        case "vod":
